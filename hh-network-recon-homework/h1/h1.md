...
Edit: 
- You can find JA4 and JA3 hash which is a unique identifier for client SSL/TLS config
...
Edit: According to my friend Andro's writeup, we were supposed to look up the network interface card type using the mac address which turned out to be a virtual mac address, so it's some sort of vitual machine network interface card.
...
AndroOtt: https://github.com/AndroOtt/Verkkoontunkeutuminen/blob/main/h1.md